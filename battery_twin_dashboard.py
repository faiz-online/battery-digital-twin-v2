...
with tab1:
    ...
    def soh_at(n):
        return physics_soh(n, snap['ambient_temp'], 0.85, 30)
...
# ── Run detector across the generated readings ────────────────────────────
    ...
    # Dynamic degradation scenario
actual_soh = rng.uniform(70, 98)
cycle_number = rng.randint(100, 900)
...
def health_dot(score):
    ...
    h1.markdown(
    f"**Battery**<br><span style='font-size:28px;'>{health_dot(100-cat_counts['Health']*20)}</span>",
    unsafe_allow_html=True
)

h2.markdown(
    f"**Thermal**<br><span style='font-size:28px;'>{health_dot(100-cat_counts['Thermal']*15)}</span>",
    unsafe_allow_html=True
)

h3.markdown(
    f"**Sensors**<br><span style='font-size:28px;'>{health_dot(100-cat_counts['Sensor']*10)}</span>",
    unsafe_allow_html=True
)

h4.markdown(
    f"**BMS**<br><span style='font-size:28px;'>{health_dot(100-cat_counts['Degradation']*25)}</span>",
    unsafe_allow_html=True
)
